...
if [ "$L1" -eq 0 ] && [ "$L2" -eq 0 ] && [ "$L3" -eq 0 ]; then
  ...
  echo "RPM_URL=$SECUREBLUE_REPO_BASEURL/${RPM_LOC:-Packages/trivalent-$VR.$ARCH.rpm}"
  echo "SRI=$(nix hash convert --hash-algo sha256 --to sri "$RPM_SHA256" 2>/dev/null || nix hash to-sri --type sha256 "$RPM_SHA256")"
  exit 0
fi
